...
Shader "Clayxel/ClayxelPickingShader" {
	SubShader {
		...
		Pass {
			...
			struct VertexData{
				...
				int2 solidId: TEXCOORD1;
			};

			struct FragData{
				fixed4 selection: SV_TARGET0;
			};
			...
			VertexData vert(uint id : SV_VertexID){
				...
				float3 camUpVec = float3(unity_CameraToWorld[0][1], unity_CameraToWorld[1][1], unity_CameraToWorld[2][1]) * splatRadius;
				float3 camSideVec = float3(unity_CameraToWorld[0][0], unity_CameraToWorld[1][0], unity_CameraToWorld[2][0]) * (splatRadius * 2.0);
				...
				outVertex.solidId = int2(compressedData.w, 0);
			...
			FragData frag(VertexData inVertex){
				...
				outData.selection = float4(unpackRgb(inVertex.solidId.x), clayxelIdA);
