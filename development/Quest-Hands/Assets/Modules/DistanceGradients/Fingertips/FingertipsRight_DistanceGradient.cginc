...
void _FingertipsRight_DistanceGradient_float (float3 worldPosition, float distanceMultiplier, out float distance, out float influence)
{
    distance = 10000; 
    influence = 0;
    ...
    for(int i = 0; i < _FingertipsRight_DistanceGradientCentersLength; i++) {  
        ...
        float dist = length(worldPosition - vec.xyz) / vec.w * distanceMultiplier;
